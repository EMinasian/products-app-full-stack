...
export default function AuthPage() {
  ...
  const [formState, formAction] = useActionState(
    isLoginView(view) ? authenticateUser : createUser,
    { errors: [] },
  );

  return (
    <Box component="form" autoComplete="off" width={600} action={formAction}>
      <Stack spacing={2}>
        {!isLoginView(view) && (
          <>
        ...
        <Link
          component={NextLink}
          href={isLoginView(view) ? "/auth?view=signup" : "/auth?view=login"}
          variant="body2"
          align="center"
        >
          {isLoginView(view)
